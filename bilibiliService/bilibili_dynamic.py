"""
Python script to get bilibili dynamic activity.
"""
...
class BilibiliDynamic:
    """
    Init the base url for the bilibili dynamic api to fetch content.
    """
    def __init__(self, uuid: str):
        self.uid = uuid
        self.base_url = 'https://api.vc.bilibili.com/dynamic_svr/v1/' \
                        'dynamic_svr/space_history?visitor_uid=0&' \
                        f'host_uid={uuid}&offset_dynamic_id=0'
    ...
    def _get_dict(self):
        """
        Private method to get the json data to dictionary.
        :return: dict, contains the dynamic activities.
        """
        try:
            page = requests.get(self.base_url, headers=HEADERS).content.decode('utf-8')
        except Exception as err:
            print('Error occurred when getting dynamic %s' % err)
            return {'-1': ''}
    ...
    def _get_last_content(self):
        """
        Get the most recent content of the dyanmic activitity.
        :return: str, the recent content in natural language.
        """
        response = ''
    ...
    def _get_dynamic_pictures(self):
        """
        Download image from the dynamic, if any.
        :return: str, path for the downloaded image.
        """
        img_path = []
        try:
        ...
        for picture in dict(pictures):
            ...
            try:
                ...
                if not os.path.exists(file_name):
                    with open(file_name, 'wb') as file_opened:
                        file_opened.write(response.content)

                img_path.append(file_name)

            except Exception as err:
                print('Error occurred when getting picture %s' % err)

        return img_path

    def _get_origin_dict(self):
        """
        Get the origin post from a re-post dynamic activity if any.
        :return: dict, origin post if any.
        """
        try:
    ...
    def get_last_content(self) -> str:
        """
        Convert the latest dynamic activity to human readable string.
        Also this method will convert the image path to CQ code.
        :return: str, if have image, the image's path will convert to CQ code.
        """
